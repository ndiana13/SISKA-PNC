...
function beri_no_sk($data) {
	global $conn;	
	$tgl     = date("Y-m-d H:i:s");
	$id_sp 	 = htmlspecialchars($data["id_sp"]);
	$no_sk   = htmlspecialchars($data['no_sk']);

	$cek_no_sk = mysqli_query($conn, "SELECT * FROM tb_sk WHERE no_sk = '$no_sk'");
	...
	$query ="UPDATE tb_sp,tb_sk SET tb_sp.status_sp = '9', tb_sp.tgl_umum='$tgl', tb_sk.no_sk='$no_sk' WHERE tb_sp.id_sp='$id_sp' AND tb_sk.id_sp ='$id_sp'";
