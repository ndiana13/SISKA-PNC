...
function acc_ketuabaak($id_sp) {
	global $conn;	
	$tgl = date("Y-m-d H:i:s");

	$query ="UPDATE tb_sp SET status_sp = '3', tgl_baak = '$tgl' WHERE id_sp = $id_sp";

	mysqli_query($conn,$query);
	return mysqli_affected_rows($conn);	
}
function dec_ketuabaak($data) {
	global $conn;	
	$tgl     = date("Y-m-d H:i:s");
	$id_sp 	 = htmlspecialchars($data["id_sp"]);
	$catatan = htmlspecialchars($data['catatan']);

	$query ="UPDATE tb_sp,tb_sk SET tb_sp.status_sp = '4', tb_sp.tgl_baak='$tgl', tb_sk.catatan='$catatan' WHERE tb_sp.id_sp='$id_sp' AND tb_sk.id_sp ='$id_sp'";
